...
import type { Mapper } from './mapper/MongooseBaseMapper';
import type { BaseRepositoryPort } from '@shared/ports/persistence/BaseRepositoryPort';
  ...
  implements BaseRepositoryPort<TDomain>
{
  ...
  async updateMany(
    ...
    const operations: AnyBulkWriteOperation<unknown>[] = updates.map(
      (update) => ({
        updateOne: {
