...
import { NotificationServiceEventSubscribers } from '@infrastructure/event/NotificationServiceEventSubscribers';
...
export class NotificationService {
  ...
  private async configureMessagaseBroker(): Promise<void> {
    ...
    const rabbitMQConfigurer = new RabbitMQConfigurer(
      rabbitMqConnection,
      new RabbitMQqueueFormatter('notification-service'),
      3,
      container.get<LoggerPort>(SharedCoreDITokens.Logger),
    );

    const subscribers = NotificationServiceEventSubscribers.from(container);
