  ...
  implements HandleExpiredPendingPaymentsUseCase
{
  ...
  async execute(): Promise<void> {
    ...
    for (const session of pendingSessions) {
      ...
      if (session.availabilitySlotId) {
        const slot = slotsMap.get(session.availabilitySlotId);
        if (slot) {
          slotIdsToUpdate.push(slot.id);
        } else {
    ...
    try {
      await this.uow.runTransaction(async (trx) => {
        ...
        if (slotIdsToUpdate.length > 0) {
          await trx.slotRepository.updateAllSlotStatus(
            { ids: slotIdsToUpdate },
            SlotStatus.AVAILABLE,
          );
          this.logger.info(
            `Successfully marked ${slotIdsToUpdate.length} slots as available.`,
          );
        }
