...
import { GATEWAY } from '@/utils/constants/service';
...
const devConsoleFormat = printf(
  ({ level, message, timestamp, correlationId, context, ...info }) => {
    ...
    return `[${GATEWAY}] [${timestamp as string}] ${correlationPart} [${(context as string) ?? GATEWAY}] ${level.toUpperCase()}: ${message as string} ${meta}`;
  },
);
