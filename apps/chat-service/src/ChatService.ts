...
import { ChatServiceEventSubscribers } from '@infrastructure/event/ChatServiceEventSubscribers';
...
export class ChatService {
  ...
  private async configureMessagaseBroker(): Promise<void> {
    ...
    const rabbitMQConfigurer = new RabbitMQConfigurer(
      rabbitMqConnection,
      new RabbitMQqueueFormatter('chat-service'),
      3,
      container.get<LoggerPort>(SharedCoreDITokens.Logger),
    );

    const subscribers = ChatServiceEventSubscribers.from(container);
