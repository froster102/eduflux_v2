...
const devConsoleFormat = printf(
  ({ level, message, timestamp, correlationId, context, ...info }) => {
    ...
    return `[${COURSE_SERVICE}] [${timestamp as string}] ${correlationPart} [${(context as string) ?? COURSE_SERVICE}] ${level.toUpperCase()}: ${message as string} ${meta}`;
  },
);
      ...
      format: combine(
        winston.format((info) => {
          ...
          if (store && store.has('correlationId')) {
            ...
            info.application = COURSE_SERVICE;
