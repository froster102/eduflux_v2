...
const devConsoleFormat = printf(
  ({ level, message, timestamp, correlationId, context, ...info }) => {
    ...
    return `[${ENROLLMENT_SERVICE}] [${timestamp as string}] ${correlationPart} [${(context as string) ?? ENROLLMENT_SERVICE}] ${level.toUpperCase()}: ${message as string} ${meta}`;
  },
);
...
export class WinstonLoggerAdapter implements LoggerPort {
      ...
      format: combine(
        winston.format((info) => {
          ...
          if (store && store.has('correlationId')) {
            ...
            info.application = ENROLLMENT_SERVICE;
            info.enviroment = envVariables.NODE_ENV;
          }
          return info;
        })(),
        isDevelopment
          ? timestamp({ format: istTimestampFormat })
          : timestamp({ format: 'YYYY-MM-DDTHH:mm:ss.SSSZ' }),
        isDevelopment ? devConsoleFormat : prodJsonFormat,
        isDevelopment ? colorize({ all: true }) : winston.format.uncolorize(),
      ),
      transports: [
        new winston.transports.Console({
          format: isDevelopment ? undefined : prodJsonFormat,
        }),
      ],
      exitOnError: false,
    });
  ...
  fromContext(contextName: string): LoggerPort {
    return new WinstonLoggerAdapter(contextName);
  }
}
