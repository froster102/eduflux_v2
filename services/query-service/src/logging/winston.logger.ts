...
const devConsoleFormat = printf(
  ({ level, message, timestamp, correlationId, context, ...info }) => {
    ...
    return `[${QUERY_SERVICE}] [${timestamp as string}] ${correlationPart} [${(context as string) ?? QUERY_SERVICE}] ${level.toUpperCase()}: ${message as string} ${meta}`;
  },
);
...
export class WinstonLogger {
      ...
      format: combine(
        winston.format((info) => {
          ...
          if (store && store.has("correlationId")) {
            ...
            info.application = QUERY_SERVICE;
  ...
  fromContext(contextName: string) {
